...
The installer generates `.env` from `.env.example`, then passes an install config file to the DB bootstrap command described in `docs/productization/02-bootstrap-install.md`. Do not put real secrets in `.env.example`, docs, GitHub issues, logs, or installer summaries.
...
- upload limits
- private backup/restore paths and limits
...
- `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_HOST`, and `POSTGRES_PORT` when using PostgreSQL
...
PostgreSQL is recommended for production installs. SQLite remains supported for development, tests, small installs, and fallback. Existing production installs are not migrated automatically by this phase; SQLite to PostgreSQL migration is documented separately in `docs/productization/POSTGRES_MIGRATION.md`.
...
| `POSTGRES_USER` | postgres only | `qasedak` | PostgreSQL role used by Django. | no | yes |
| `POSTGRES_PASSWORD` | postgres only | `generate-with-installer` | Runtime DB password generated by the installer and stored only in `.env`. | yes | generate |
| `POSTGRES_HOST` | postgres only | `127.0.0.1` | PostgreSQL host. | no | yes |
| `POSTGRES_PORT` | postgres only | `5432` | PostgreSQL port. | no | yes |
| `POSTGRES_CONN_MAX_AGE` | no | `60` | Django persistent connection lifetime in seconds. | no | advanced |
| `POSTGRES_SSLMODE` | no | `prefer` | libpq SSL mode passed to Django's PostgreSQL backend. | no | advanced |
...
| `QASEDAK_PRIVATE_BACKUP_ROOT` | no | `/opt/qasedak/backups/admin_center` | Private directory for Admin-created backup archives. Never serve publicly. | no | advanced |
| `QASEDAK_RESTORE_UPLOAD_ROOT` | no | `/opt/qasedak/backups/restore_uploads` | Private directory for uploaded restore packages and validation temp files. | no | advanced |
| `QASEDAK_BACKUP_MAX_UPLOAD_SIZE` | no | `536870912` | Maximum uploaded backup archive size in bytes. | no | advanced |
| `QASEDAK_BACKUP_MAX_EXTRACTED_SIZE` | no | `2147483648` | Maximum safe extracted payload size in bytes. | no | advanced |
| `QASEDAK_ADMIN_RESTORE_ENABLED` | no | `False` | Dangerous future flag. P1 keeps destructive Admin apply disabled; use command-based restore validation. | no | advanced |
