...
from .rules import RetentionRuleEngine
...
class RetentionEngine:
    ...
    def __init__(self, rule_engine=None, experiment_engine=None, offer_selector=None, tracker=None, ai_optimizer=None):
        self.rule_engine = rule_engine or RetentionRuleEngine()
    ...
    def _optimize_offer(self, decision, user, context):
        try:
            ...
            logger.warning(
                "AI retention optimization fallback user=%s event=%s: %s",
    ...
    def _select_variant(self, decision, user, context):
        try:
            ...
            logger.warning(
                "Retention optimization fallback user=%s event=%s: %s",
                getattr(user, "pk", None),
                context.get("event_type"),
                exc,
            )
